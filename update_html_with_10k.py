...
# -*- coding: utf-8 -*-
"""
把 xieyin_results_10k.json 的 427 条全量数据完整内嵌写入 index.html
"""

import json

with open("xieyin_results_10k.json", "r", encoding="utf-8") as f:
    data_10k = json.load(f)

all_items = []
for doc, items in data_10k.items():
    for item in items:
        all_items.append({
            "doc": doc,
...
<head>
  ...
  <title>Codex Resets — 10,000完整词库 典籍谐音梗追踪器</title>
...
<div class="page">
  <header class="masthead">
    <div class="masthead-brand">
      ...
      <div>
        <h1 class="masthead-title">Codex Resets · 10,000完整词库</h1>
        <p class="masthead-tagline">完整扩充 10,000 个现代常用词汇 &middot; 大扫描产出 427 条典籍谐音梗</p>
      </div>
    </div>
  </header>
  ...
  <main>
    <section class="hero">
      <p class="hero-explainer">
        已加载 <strong>10,000个现代常用词汇全量词库</strong>。严守 <strong>无跨句切片</strong> 与 <strong>N字对N字全音节对齐</strong>。
      </p>

      <div class="hero-card">
        <span class="hero-label">10,000词库大扫描示范</span>
        <span class="hero-figure">把酒问【晴天】。不知天上宫阙...</span>
        <p class="hero-sub">《水调歌头》「青天」(qīng tiān) ── 周杰伦金曲《晴天》(qíng tiān)</p>
      </div>
    ...
    <section class="filter-card">
      ...
      <div class="category-pills" id="categoryPills">
        ...
        <button class="pill-btn" data-cat="《道德经八十一章全本》">《道德经》 (54条)</button>
        <button class="pill-btn" data-cat="《历代名篇辞赋》">《名篇辞赋》 (46条)</button>
      </div>
    </section>

    <div class="section-head">
      <h2>10,000词库典籍谐音梗日志</h2>
...
<script>
  ...
  function render() {
    ...
    if (pageItems.length === 0) {
      punList.innerHTML = `<div style="text-align:center; padding: 50px; color: var(--ink-3); font-family: var(--font-mono);">未搜索到符合条件的 10,000 词库谐音梗数据</div>`;
      return;
    }
...
print(f"成功将 10,000 词库扫描出的 {total_count} 条全量梗数据硬编码内嵌写入 index.html！")
