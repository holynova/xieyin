...
# -*- coding: utf-8 -*-
"""
把 xieyin_results_thuocl.json 的 749 条 THUOCL 清华网络权威词库全量数据完整内嵌写入 index.html
"""

import json

with open("xieyin_results_thuocl.json", "r", encoding="utf-8") as f:
    data_thuocl = json.load(f)

all_items = []
for doc, items in data_thuocl.items():
    for item in items:
        all_items.append({
            "doc": doc,
...
<head>
  ...
  <title>Codex Resets — 清华THUOCL网络权威词库 典籍谐音梗</title>
...
<div class="page">
  <header class="masthead">
    <div class="masthead-brand">
      ...
      <div>
        <h1 class="masthead-title">Codex Resets · THUOCL清华网络词库</h1>
        <p class="masthead-tagline">全量源自清华大学 THUOCL 官方网络热词/名人/影视/歌曲词库 &middot; 零人造词</p>
      </div>
    </div>
  </header>
  ...
  <main>
    <section class="hero">
      <p class="hero-explainer">
        已接入清华大学 THUOCL 官方网络热词与知名名人/影视词库。严守 <strong>无跨句切片</strong> 与 <strong>N字对N字全音节对齐</strong>。
      </p>

      <div class="hero-card">
        <span class="hero-label">网络词库名篇反差梗示范</span>
        <span class="hero-figure">落霞与【鼓舞】齐飞，秋水共长天一色。</span>
        <p class="hero-sub">《滕王阁序》「孤鹜」(gū wù) ── 清华网络词库《鼓舞》(gǔ wǔ)</p>
      </div>
      ...
      <dl class="stat-row">
        ...
        <div class="stat-tile stat-tile--rose">
          <dt>完全同音同调梗</dt>
          <dd class="mono" id="statSameTone">58</dd>
        </div>
        <div class="stat-tile stat-tile--sky">
          <dt>THUOCL开源词汇量</dt>
          <dd class="mono">15,000+</dd>
        </div>
      </dl>
    </section>

    <section class="filter-card">
      <input type="text" class="search-input" id="searchInput" placeholder="搜索网络热词梗（如：晴天、鼓舞、起飞、犹豫、由于、宇宙、加仓）、名句...">
      <div class="category-pills" id="categoryPills">
        <button class="pill-btn active" data-cat="ALL">全部典籍 (""" + str(total_count) + """条)</button>
        <button class="pill-btn" data-cat="《唐诗名篇全本》">《唐诗名篇》 (324条)</button>
        <button class="pill-btn" data-cat="《宋词名篇全本》">《宋词名篇》 (148条)</button>
        <button class="pill-btn" data-cat="《历代名篇辞赋》">《名篇辞赋》 (92条)</button>
        <button class="pill-btn" data-cat="《道德经八十一章全本》">《道德经》 (126条)</button>
        <button class="pill-btn" data-cat="《诗经全集名篇》">《诗经全集》 (35条)</button>
        <button class="pill-btn" data-cat="《论语全篇精选》">《论语全篇》 (24条)</button>
      </div>
    </section>

    <div class="section-head">
      <h2>清华THUOCL网络词库典籍谐音梗日志</h2>
...
<script>
  ...
  function render() {
    ...
    if (pageItems.length === 0) {
      punList.innerHTML = `<div style="text-align:center; padding: 50px; color: var(--ink-3); font-family: var(--font-mono);">未搜索到符合条件的网络词库典籍谐音梗</div>`;
      return;
    }
      ...
      return `
        <div class="log-card">
          ...
          <div class="mono-breakdown">
            <div>
              切片 <strong>${item.oText}</strong> (${item.pyO}) ──▶ 网络开源词 <strong>${item.kw}</strong> (${item.pyT})
            </div>
...
print(f"成功将清华 THUOCL 网络权威词库扫描出的 {total_count} 条全量梗数据硬编码内嵌写入 index.html！")
