...
# -*- coding: utf-8 -*-
"""
前端 HTML 网页构建器 (src/builder.py)
职责：读取挖掘出的 JSON 谐音梗数据，构建单文件离线 HTML 网页（100% Codex-Resets 视觉）
"""
...
def build_html(
    json_path="dist/xieyin_results.json",
    output_html_path="dist/index.html",
    copy_to_root=True,
):
    print(f"[Builder] 正在读取 JSON 结果: {json_path} ...")
    with open(json_path, "r", encoding="utf-8") as f:
        data_json = json.load(f)

    all_items = []
    for doc, items in data_json.items():
        for item in items:
            all_items.append(
                {
                    "doc": doc,
                    "orig": item["pun_sentence"].replace(
                        f"【{item['replaced_word']}】", item["original_text"]
                    ),
    ...
    same_tone_count = len([i for i in all_items if i["sameTone"]])
...
<div class="page">
  <header class="masthead">
    <div class="masthead-brand">
      ...
      <div>
        <h1 class="masthead-title">Codex Resets · 古籍谐音梗追踪器</h1>
        <p class="masthead-tagline">单文件离线全量数据内嵌 &middot; 自动构建产物</p>
      </div>
    </div>
  </header>
  ...
  <main>
    <section class="hero">
      <p class="hero-explainer">
        严格执行 <strong>不跨标点断句</strong> 与 <strong>N字对N字全音节对齐法则</strong>。
      </p>
      ...
      <dl class="stat-row">
        <div class="stat-tile stat-tile--sun">
          <dt>扫描发现总梗数</dt>
          <dd class="mono" id="statCount">{total_count}</dd>
        </div>
        <div class="stat-tile stat-tile--rose">
          <dt>完全同音同调梗</dt>
          <dd class="mono" id="statSameTone">{same_tone_count}</dd>
        </div>
    ...
    <section class="filter-card">
      ...
      <div class="category-pills" id="categoryPills">
        <button class="pill-btn active" data-cat="ALL">全部典籍 ({total_count}条)</button>
...
"""
    ...
    print(f"[Builder] 🎉 成功生成单文件 HTML 网页: {output_html_path}！")

    if copy_to_root:
        with open("index.html", "w", encoding="utf-8") as f:
            f.write(html_content)
        print("[Builder] 已同步更新根目录 index.html！")
