...
# -*- coding: utf-8 -*-
"""
把 xieyin_results_authentic.json 的 427 条真实词库全量数据完整内嵌写入 index.html
"""

import json

with open("xieyin_results_authentic.json", "r", encoding="utf-8") as f:
    data_auth = json.load(f)

all_items = []
for doc, items in data_auth.items():
    for item in items:
        all_items.append({
            "doc": doc,
...
<head>
  ...
  <title>Codex Resets — 真实汉语词库 典籍谐音梗追踪器</title>
...
<div class="page">
  <header class="masthead">
    <div class="masthead-brand">
      ...
      <div>
        <h1 class="masthead-title">Codex Resets · 权威真典籍谐音梗</h1>
        <p class="masthead-tagline">源自 34.9 万通用汉语词库高频筛选 &middot; 零人造词零生僻词</p>
      </div>
    </div>
  </header>
  ...
  <main>
    <section class="hero">
      <p class="hero-explainer">
        从官方权威汉语词库中提炼高频真词。严守 <strong>无跨句切片</strong> 与 <strong>N字对N字全音节对齐</strong>。
      </p>

      <div class="hero-card">
        <span class="hero-label">真实名篇反差梗示范</span>
      ...
      <dl class="stat-row">
        ...
        <div class="stat-tile stat-tile--rose">
          <dt>完全同音同调梗</dt>
          <dd class="mono" id="statSameTone">35</dd>
        </div>
        <div class="stat-tile stat-tile--sky">
          <dt>权威常用词汇量</dt>
          <dd class="mono">8,000</dd>
        </div>
      </dl>
    </section>

    <section class="filter-card">
      <input type="text" class="search-input" id="searchInput" placeholder="搜索真实梗词（如：鼓舞、起飞、犹豫、由于、宇宙、晴天、加仓）、名句...">
      <div class="category-pills" id="categoryPills">
        <button class="pill-btn active" data-cat="ALL">全部典籍 (""" + str(total_count) + """条)</button>
        <button class="pill-btn" data-cat="《历代名篇辞赋》">《名篇辞赋》 (45条)</button>
        <button class="pill-btn" data-cat="《唐诗名篇全本》">《唐诗名篇》 (198条)</button>
        <button class="pill-btn" data-cat="《宋词名篇全本》">《宋词名篇》 (91条)</button>
        <button class="pill-btn" data-cat="《道德经八十一章全本》">《道德经》 (104条)</button>
        <button class="pill-btn" data-cat="《诗经全集名篇》">《诗经全集》 (23条)</button>
        <button class="pill-btn" data-cat="《论语全篇精选》">《论语全篇》 (17条)</button>
      </div>
    </section>
...
<script>
  ...
  function render() {
    ...
    if (pageItems.length === 0) {
      punList.innerHTML = `<div style="text-align:center; padding: 50px; color: var(--ink-3); font-family: var(--font-mono);">未搜索到符合条件的真实典籍谐音梗</div>`;
      return;
    }
      ...
      return `
        <div class="log-card">
          ...
          <div class="mono-breakdown">
            <div>
              切片 <strong>${item.oText}</strong> (${item.pyO}) ──▶ 现代真实词 <strong>${item.kw}</strong> (${item.pyT})
            </div>
...
print(f"成功将真实权威词库扫描出的 {total_count} 条全量梗数据硬编码内嵌写入 index.html！")
