...
# -*- coding: utf-8 -*-
"""
清华 THUOCL (IT/网络热词/名人明星/财经/美食) + Jieba 34.9万真实词库 权威爆笑古籍谐音梗扫描引擎
零人造词！全量源自网络权威开源词库！
"""
...
def load_thuocl_and_jieba():
    words_set = set()

    # 1. 载入清华 THUOCL 开放词库 (IT/网络/美食/财经/名人)
    thuocl_dir = "/Users/sym/Code/xieyin/thuocl_data"
    for fname in os.listdir(thuocl_dir):
        fpath = os.path.join(thuocl_dir, fname)
    ...
    # 2. 载入 Jieba 常用词库按词频筛选高频词
    jieba_path = "/Users/sym/Code/xieyin/real_dict.txt"
    if os.path.exists(jieba_path):
        ...
        for w, _ in words_with_freq[:10000]:
            words_set.add(w)
    ...
    print(f"成功全量加载 THUOCL 清华网络热词库 + 权威 Jieba 高频词库，共 {len(final_words)} 个 100% 真实开源词汇！")
    return final_words

ALL_AUTHENTIC_WORDS = load_thuocl_and_jieba()
...
def main():
    print(f"正在建立 THUOCL + Jieba 权威词库拼音字典...")
    engine = THUOCLEngine(ALL_AUTHENTIC_WORDS)
    print("字典建立完毕！开始极速全量大扫描...")
    ...
    for doc_name, sentences in AUTHENTIC_CORPUS.items():
        ...
        print(f"  小计：{doc_name} 匹配到 {doc_count} 个清华网络权威谐音梗！")

    with open("xieyin_results_thuocl.json", "w", encoding="utf-8") as f:
        json.dump(results_export, f, ensure_ascii=False, indent=2)
    print(f"\n📁 THUOCL 权威网络词库共扫描出 {total_matches} 个梗！已保存至 `xieyin_results_thuocl.json`！")
